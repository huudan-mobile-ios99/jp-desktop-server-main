...
async function initializeCleanupSub2(mongoose) {
  try {
    const InfoModel = require('../model/information_model_sub2');
    const HitModel = require('../model/hit_model_sub2');
      ...
      try {
       // Check MongoDB connection before running cleanup
        if (mongoose.connection.readyState !== 1) {
          console.warn('⚠️cleanup_sub2 MongoDB not connected. Attempting to reconnect...');
        ...
        if (broadcastCount > 25) {
          ...
          console.log(`🗑 Deleted ${excessBroadcasts} oldest InformationBroadcast records to maintain 25 latest records`);
        } else {
          console.log('✅ No cleanup needed: InformationBroadcast collection has 25 or fewer records');
        }
        ...
        if (hitCount > 75) {
          ...
          console.log(`Deleted ${excessHits} oldest Hits records to maintain 25 latest records`);
        } else {
          console.log('No cleanup needed: Hits collection has 25 or fewer records');
        }
      } catch (error) {
        console.error('Error during scheduled cleanup:', error);
      }
    });
    console.log('Cleanup job scheduled to run every 1 hour to maintain 25 latest records for InformationBroadcast and Hits');
  } catch (error) {
...
module.exports = { initializeCleanupSub2 };
