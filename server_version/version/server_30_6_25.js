...
const io = require('socket.io')(server, {
  pingInterval: 15000, // Send pings every 15 seconds
  pingTimeout: 5000, // Close if no pong in 5 seconds
...
const endpoints = [
  ...
  'ws://192.168.100.202/Interfaces/Media/JackpotsGateway?COMPUTERNAME=TECHVG',
  'ws://192.168.100.202/Interfaces/Media/JackpotsGateway?COMPUTERNAME=MAINTECH',
];
let currentEndpointIndex = 0;
let isConnecting = false;
let reconnectTimer;
const reconnectDelay =2 * 60 * 1000; // 2 minutes in milliseconds
const initialConnectDelay = 150; // 0.1 seconds (100ms) to match Dart client
var hitsdb = [];
let publisher;
let wss;
let client;
...
function connect() {
  ...
  // Add 0.1-second delay before connecting
  setTimeout(() => {
    ...
    client.on('open', function() {
      ...
      const pingInterval = setInterval(() => {
        if (client.readyState === WebSocket1.OPEN) {
          client.ping();
          console.log(`Sent ping to ${endpoint}`);
        } else {
    ...
    client.on('error', function(error) {
      console.error(`Error connecting to ${endpoint}:`, error.message || error);
    ...
    client.on('close', function(message) {
      ...
      isConnecting = false;
      // Only switch to next endpoint if close code is 1006
      if (message.code === 1006) {
        currentEndpointIndex = (currentEndpointIndex + 1) % endpoints.length;
        console.log(`Close code 1006 detected, switching to next endpoint: ${endpoints[currentEndpointIndex]}`);
      }
      // Schedule reconnection (to current or next endpoint)
      clearTimeout(reconnectTimer);
      reconnectTimer = setTimeout(connect, reconnectDelay);
    });
  }, initialConnectDelay);
