...
function handleSocketIO(io) {
  ...
  // Handle change stream events for Hits
  hitChangeStream.on('change', async (change) => {
    try {
      if (change.operationType === 'insert') {
        ...
        // Emit to all connected clients
        io.emit('hit_latest', [latestHit]); // Wrap in array to match /hit_latest API response

        // Emit to clients subscribed to the specific jackpotId
        if (latestHit.jackpotId) {
          io.to(`hit:${latestHit.jackpotId}`).emit('hit_latest', [latestHit]);
        }
  ...
  // Handle change stream errors for Hits
  ...
  io.on('connection', (socket) => {
    ...
    // Allow clients to subscribe to specific jackpotId updates for Hits
    socket.on('subscribe_hit', (jackpotId) => {
      if (typeof jackpotId === 'string' && jackpotId) {
        socket.join(`hit:${jackpotId}`);
        console.log(`[${formatDate()}] Client ${socket.id} subscribed to hit jackpotId: ${jackpotId}`);
      }
    });
